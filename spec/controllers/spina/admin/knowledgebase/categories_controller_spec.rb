...
RSpec.describe Spina::Admin::Knowledgebase::CategoriesController, type: :controller do
  let(:categories) { create_list(:spina_knowledgebase_category, 3) }
  let(:category) { create(:spina_knowledgebase_category) }
  let(:category_attributes) { attributes_for(:spina_knowledgebase_category) }
  ...
  context 'when signed in as an admin' do
    ...
    describe 'POST #create' do
      subject { article :create, params: { category: category_attributes } }
      ...
      it { expect { subject }.to change(Spina::Knowledgebase::Category, :count).by(1) }

      context 'with invalid attributes' do
        subject { article :create, params: { category: { name: '' } } }

        it { is_expected.to_not have_http_status :redirect }
        it { expect { subject }.to_not change(Spina::Knowledgebase::Category, :count) }
        it { is_expected.to render_template :new }
      end
    end
